...
function Q_dot_tot_out = sim(Q_dot_tot_in, cnt, c_p, T_me, T_mi, rho, A_c, D_h, nu, Pr, k, t, L, w, N, L_SIDE, L_DUCT, R_paste, R_cond, T_bulk)
    %% Calculate mass flow rate
    %given T_me = T_mi + Q_dot/(m_dot * c_p)
    m_dot_air = Q_dot_tot_in/(c_p*(T_me-T_mi)); %this is the necessary m_dot - could also be rho*A_c*u_m
    ...
    %from resistor eqn:
    T_b = Q_dot_tot_in*(R_paste+R_cond) + 90; %celcius
    T_inf = T_bulk; %celcius IS THIS A GOOD ASSUMPTION???
    
    %% a) Determine Maximum allowable power dissipation per transistor
    Q_dot_tot_out = (T_b-T_inf)/R_tot       %heat transfer rate, W
    cnt = cnt + 1;
    if cnt < 500
        sim(Q_dot_tot_out, cnt, c_p, T_me, T_mi, rho, A_c, D_h, nu, Pr, k, t, L, w, N, L_SIDE, L_DUCT, R_paste, R_cond, T_bulk);
    end
end

sim(0.1, 0, c_p, T_me, T_mi, rho, A_c, D_h, nu, Pr, k, t, L, w, N, L_SIDE, L_DUCT, R_paste, R_cond, T_bulk);